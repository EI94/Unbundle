...
const baseUseCaseSchema = z.object({
  title: z.string(),
...
export async function generateUseCases(
  ...
  const schema = includeEsg
    ? z.object({ useCases: z.array(baseUseCaseSchema.merge(esgExtension)) })
    : z.object({ useCases: z.array(baseUseCaseSchema) });

  const esgRules = includeEsg
...
${esgRules} **Business case dettagliato**: numeri, metriche attese, costi evitati
${includeEsg ? "7" : "6"}. **Genera 3 categorie**:
...
${includeEsg ? "8" : "7"}. Ogni use case deve avere un timeline realistico
${includeEsg ? "9" : "8"}. Non generare più di 12 use case totali
